...
if __name__ == "__main__":
	...
	ShortCode = np.array([0x72])
	ShortCodeSize = 7
	LongCode = np.array([0x766B8C7F, 0x4259F1BA, 0x4327DC56, 0x017E2DEC])
	LongCodeSize = 31
	# LongCode = np.array([0x410C53D1C96ECD5F, 0x12AB1D4CF31A248C, 0x6642CEEBBD871EF8, 0x0F9169A520BD6A1])
	# LongCodeSize = 63

	for dist in Dist:#np.array([4]):
		...
		for itr in range(10):
			...
			if Distance[0] > 1.5:
				nCodeSample = nBitSample * LongCodeSize
				Code = LongCode
				CodeSize = LongCodeSize

			else:
				nCodeSample = nBitSample * ShortCodeSize
				Code = np.append(ShortCode, np.random.randint(0x7F, size = 3) )
				CodeSize = ShortCodeSize
			...
			# print("o Obstacle Position: x =", ObstaclePos[0], "m,  y =", ObstaclePos[1], "m")
			# print("o Distance:", Distance[0], "m")
			...
			SetDelay = Distance[0] * 2 / 340
			SetDelay = np.append(SetDelay, np.random.rand(3) * (MaxTime - MinTime - nCodeSample*Ts) + MinTime)
			# print("o Delay:", SetDelay)
			...
			# -> Make TX Signal
			for i in range(4):
				USSStream = np.append(np.zeros(nRTTSample[i]), ModData[i,:])
			...
			Attenuation = pow(10, AttenuationCoef/20 * nRTTSample[0] * Ts * 340)
			Attenuation = np.append(Attenuation,  np.random.rand(3) * (pow(10,AttenuationCoef/20 * 9)) )
			TXMatrix = TXMatrix  * np.reshape(Attenuation, (Attenuation.size, 1))

			# -> Make RX Signal
			RXSignal = TXMatrix[0,:] + TXMatrix[1,:] #+ TXMatrix[2,:] + TXMatrix[3,:]
			Noise = np.random.normal(0, 1, RXSignal.size) * np.sqrt(N0[34]/2)
			...
			# a = signal.correlate(SymbolMatrix_I[1,:], SymbolMatrix_I[0,:])
			# b = signal.correlate(SymbolMatrix_Q[1,:], SymbolMatrix_Q[0,:])
			...
			# plt.plot(tSignal, TXMatrix[0,:])
			# plt.subplot(232, sharex = axes)
			# plt.plot(tSignal, TXMatrix[1,:])
			# plt.subplot(233, sharex = axes)
			# a = np.append(SymbolMatrix_I[0], SymbolMatrix_Q[0])
			# plt.plot(tSignal, RXSignal, tSignal[nRTTSample[0]:nRTTSample[0]+SymbolMatrix_I.shape[1]*2], a)
			# plt.subplot(234, sharex = axes)
			# plt.plot(tSignal, TXMatrix[2,:])
			# plt.subplot(235, sharex = axes)
			# plt.plot(tSignal, TXMatrix[3,:])
			# plt.subplot(236, sharex = axes)
			# plt.plot(tCorrelation, Correlation)
			# plt.show()
			
			if np.abs(Estimation - Distance[0]) > 0.3:
				ErrCnt = ErrCnt + 1
