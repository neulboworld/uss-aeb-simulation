...
def BFSKmodulation(code, fc, fd, fs, bitrate)
...
# kasami-15 double
	...
	Amplitude = 12
	EbN0dB = np.arange(-10,10,1)
	SNR = pow(10,EbN0dB/10)
	N0 = np.sqrt(2) * pow(Amplitude, 4) / SNR
	...
	TrxSignal = butter_bandpass_filter(TrxSignal, fc - 1200, fc + 1200, fs, 1)
	
	# -> Insert channel property
	Attenuated = TrxSignal * pow(10,AttenuationCoef/20 * nRTTSample * Ts * 340)
	Noise = np.random.normal(0, 1, Attenuated.size) * np.sqrt(N0[15]/2)
